 ...
 * DashboardPo6
 * -------------------------------------------------------------------------------------
 * "List PO Melewati Batas Waktu" (Figma: purchase-order dashboard) — lists
...
export default function DashboardPo6() {
  ...
  const { dataDashboardPurchaseOrderOverDueDate, loadingDashboardPurchaseOrderOverDueDate } = useSelector(
    state => state.dashboard
  )

  const fetchData = (customParams = {}) => {
    dispatch(
      fetchDashboardPurchaseOrderOverDueDate({
  ...
  const handleView = row => {
    router.push(`/purchase-order/${row.code}`)
  }

  useEffect(() => {
    dispatch(fetchDashboardPurchaseOrderOverDueDate({ page: 1, limit: paginationModel.pageSize }))
  ...
  return (
    <DataTable
      ...
      rowCount={dataDashboardPurchaseOrderOverDueDate?.totalData || 0}
      ...
      rows={dataDashboardPurchaseOrderOverDueDate?.data || []}
