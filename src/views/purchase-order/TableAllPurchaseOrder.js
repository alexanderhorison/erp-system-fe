...
export default function TableAllPurchaseOrder() {
  ...
  const handleView = row => router.push(`/purchase-order/${row.code}`)
  const handleEdit = row => router.push(`/purchase-order/edit/${row.code}`)
  const handleAdd = () => router.push('/purchase-order/add')
  ...
  const handleSortModelChange = sortModel => {
    ...
    const orderBy = ['createdAt', 'approvedAt'].includes(field) ? field : 'createdAt'
              ...
              return (
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                    ...
                    <Typography noWrap sx={{ fontSize: '0.75rem', lineHeight: '16px', color: 'text.secondary' }}>
