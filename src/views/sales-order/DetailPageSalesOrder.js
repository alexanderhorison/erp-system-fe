...
// ** Sales Order only ever carries these statuses (see Status.js's original
...
/**
 * DetailPageSalesOrder
 * -------------------------------------------------------------------------------------
 * The sales-order document: company letterhead, billing info, the ordered
 * (and bartered) products, notes, totals, the payment instruction, and both
 * signature blocks. Mirrors DetailReceiveOrder.js's pattern.
 */
    ...
    >
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} sm={5}>
            ...
            >
              ...
              {data?.isLoanStockSO && (
                <Typography sx={{ ...mutedSx, fontStyle: 'italic' }}>Loan Stock SO</Typography>
              )}
            </Box>
          </Grid>
        </Grid>
      ...
      <CardContent sx={{ px: 5, py: 4 }}>
        <Grid container spacing={4}>
          <Grid item xs={12} sm={6}>
            <Typography sx={{ ...sectionLabelSx, mb: 2 }}>Tagihan Kepada</Typography>
            <Typography sx={mutedSx}>{data?.customer?.name?.toUpperCase() || ''}</Typography>
            <Typography sx={mutedSx}>{data?.customer?.address?.toUpperCase() || ''}</Typography>
            {data?.customer?.phoneNumber && <Typography sx={mutedSx}>{data.customer.phoneNumber}</Typography>}
          </Grid>
          <Grid item xs={12} sm={6} sx={{ textAlign: { xs: 'left', sm: 'right' } }}>
            <Typography sx={mutedSx}>Tgl. Jatuh Tempo</Typography>
            <Typography sx={{ ...sectionLabelSx, fontWeight: 500, mb: 2 }}>{data.dueDate}</Typography>
            <Typography sx={mutedSx}>Tgl. Pengiriman</Typography>
            <Typography sx={{ ...sectionLabelSx, fontWeight: 500 }}>
          ...
          <Table size='small'>
            <TableHead sx={{ backgroundColor: stone[100] }}>
              <TableRow>
                ...
                {isAdmin && <TableCell sx={{ ...sectionLabelSx, borderColor: colors.border }}>Modal</TableCell>}
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} sm={6}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 1 }}>
              <Typography sx={mutedSx}>Total Sales</Typography>
      ...
      <CardContent sx={{ px: 5, py: 4 }}>
        <Typography sx={{ fontSize: '0.875rem', fontWeight: 500, color: colors.foreground, mb: 2 }}>
          {data?.grandTotal < 0
            ? `${companyInfo?.ptName} harus melakukan pembayaran sebesar Rp ${priceFormat(Math.abs(data?.grandTotal))}`
            : `Customer ${data?.customer?.name?.toUpperCase() || ''} harus melakukan pembayaran sebesar Rp ${priceFormat(data?.grandTotal)}`}
        </Typography>
        <Typography sx={mutedSx}>Silahkan transfer ke rekening:</Typography>
        <Typography sx={{ fontSize: '0.875rem', fontWeight: 500, color: colors.foreground }}>
          {companyInfo?.bank}
        </Typography>
      </CardContent>
